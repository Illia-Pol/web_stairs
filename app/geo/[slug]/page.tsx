...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  if (!geo) {
    return createPageMetadata({
      ...
      pathname: "/contacts",
      title: `Город не найден | ${site.brand.name}`,
      description: "Страница города не найдена."
    });
  }
  ...
  return createPageMetadata({
    ...
    title: geo.seoTitle,
    description: geo.seoDescription
  });
}
...
export default function GeoPage({ params }: PageProps) {
  ...
  const serviceSchema = serviceJsonLd({
    name: `${site.brand.name} — ${geo.city}`,
    baseUrl: site.baseUrl,
    description: geo.description,
    serviceType: "Бетонные монолитные лестницы",
    areaServed: geo.city
  });
        ...
        <Container>
          <Breadcrumbs
            items={[
              ...
              { label: geo.city, href: `/geo/${geo.slug}` }
            ]}
          />

          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <h2 className="font-heading text-3xl uppercase text-coal">Локальные условия</h2>
              <p className="mt-3 text-sm text-slate-700">{geo.transport}</p>
            ...
            <Card className="bg-[#fffaf1]">
              <h2 className="font-heading text-3xl uppercase text-coal">Получить расчет</h2>
              <p className="mt-3 text-sm text-slate-700">Отправьте исходные данные и получите ориентир стоимости по вашему городу.</p>
              <div className="mt-4">
          ...
          <Card className="mt-6">
            <h2 className="font-heading text-3xl uppercase text-coal">Кейсы в городе {geo.city}</h2>
