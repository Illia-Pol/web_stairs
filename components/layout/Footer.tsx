...
const links = [
  ...
  { href: "/after-finishing", label: "После заливки" },
  { href: "/faq", label: "FAQ" },
    ...
    <footer className="border-t border-slate-200 bg-[#f8f6f1] pb-20 pt-12 sm:pb-12">
      <div className="mx-auto grid w-full max-w-6xl gap-8 px-4 sm:grid-cols-[1.3fr_1fr_1fr] sm:px-6">
        <div>
          ...
          <p className="mt-3 max-w-sm text-sm text-slate-600">{site.brand.tagline}</p>
          <p className="mt-4 text-sm text-slate-600">Основатель: {site.brand.founder}</p>
        </div>

        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Навигация</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Контакты</p>
          <ul className="mt-3 space-y-2 text-sm text-coal">
            <li>
              Телефон: <a href={`tel:${site.contacts.phoneMain}`}>{site.contacts.phoneMain}</a>
            </li>
            <li>
              Email: <a href={`mailto:${site.contacts.email}`}>{site.contacts.email}</a>
            </li>
            <li>УНП: {site.legal.unp}</li>
